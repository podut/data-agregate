...
class IngestApiService @Inject constructor(
    ...
    suspend fun getDigest(deviceId: String? = null, excludeSeen: Boolean = false): Result<DigestResponse> {
        return try {
            val response: HttpResponse = client.get("$BASE_URL/digest") {
                if (deviceId != null) parameter("deviceId", deviceId)
                if (excludeSeen) parameter("exclude_seen", true)
            }
    ...
    suspend fun getTrending(deviceId: String? = null, hours: Int = 48, limit: Int = 20, categories: String? = null): Result<List<TechNews>> = try {
        val r: HttpResponse = client.get("$BASE_URL/trending") {
            if (deviceId != null) parameter("deviceId", deviceId)
            parameter("hours", hours)
            parameter("limit", limit)
            if (categories != null) parameter("categories", categories)
        }
        if (r.status.isSuccess()) Result.success(r.body())
        else Result.failure(Exception("Error: ${r.status}"))
    } catch (e: Exception) { Result.failure(e) }

    suspend fun getDefaultFeeds(categories: String): Result<Map<String, List<String>>> = try {
        val r: HttpResponse = client.get("$BASE_URL/categories/defaults") {
